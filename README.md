...
- **🎛️ Clean, Draggable UI**: A sleek, dark-themed menu with rounded corners that can be dragged absolutely anywhere on your screen.
- **👁️ Concealable**: Easily hide the interface so it doesn't clutter your gameplay. This script uses executor GUI protection (like `syn.protect_gui`) to hide from in-game anti-cheats as well.
- **⏱️ Custom Intervals**: Set exactly how many seconds you want between each click loop depending on what the game requires.
...
| Key | Function |
| :--- | :--- |
| **`[F]`** | Toggles the auto-clicker ON and OFF playfully without needing to click the menu. |
| **`[Left/Right Shift]`** | Completely hides or shows the Auto-Clicker menu so it's not in your way. |
| **`[Click & Drag]`** | Click on the dark background of the menu to reposition it anywhere. |
